...
{ 
	while (isEmpty() == false)
	{
		deleteFirst(); // deletes the first element of the list until it is empty.
	}
}
...
void LinkedList::displayList(ofstream& fout) 
{
	...
	if (isEmpty())
	{
		cout << "[EMPTY]" << endl; //empty spot in the table
	}
	else
	{
		//cout << " ";
		...
		{
			fout << currentElement->book.getKey() + " " << currentElement->book.getName() + " "; //output to file
			fout << currentElement->book.getAuthor() + " " << currentElement->book.getVol() << " ";
			
			cout << currentElement->book.getKey() + " " << currentElement->book.getName() + " "; //output to program
			cout << currentElement->book.getAuthor() + " " << currentElement->book.getVol() << " ";
...
{
	cout << "Adding to book to list..." << endl;
...
void LinkedList::deleteAt(int pos, Element book)
{
	if (pos > count || pos < 1)
		throw outOfRange();
	else if (pos == 1)
